...
# create new column where non diapausers are 42, diapausers are days to diapause
diapauseData3 <- diapauseData3 %>% mutate(time_diap = replace(time, diapause == 0, 42))
...
# Reaction Norm Plot
ggplot(data = emoutmodAllSamp_df, aes(x = treatment, y = rate, fill = core_edge)) +
  annotate('rect', xmin = c(0.8,1.8), xmax = c(1.2, 2.2), ymin = 41, ymax = 43, alpha = 0.2) +
  ...
  annotate("text", x = c(1,2), y = 44.5, label = "Non-Diapausers") +
  labs(x = "Test Environment", y = "Days to Diapause (95% CI)", fill = "Collection \nLocation") +
  scale_x_discrete(labels = c('long' = "North Fall", 'short' = "South Fall")) +
  scale_fill_manual(values = c('core' = 'white', 'edge' = 'black'), labels = c('edge' = "South", 'core' = "North")) +
  theme_bw(base_size = 15)
...
ggplot(data = emoutAllSampPops_df) +
  annotate('rect', xmin = 0.75, xmax = 8.25, ymin = 41, ymax = 43, alpha = 0.2) +
  ...
  annotate("text", x = 4.5, y = 44, label = "Non-Diapausers") +
  labs(x = "Collection Location (N to S)", y = "Days to Diapause (95% CI)", fill = "Collection \nLocation") +
  scale_x_discrete(labels = c('Lo' = 'A', 'Hu' = 'B', 'De' = 'C', 'Pu' = 'D', 'Lc' = 'E', 'Wi' = 'F', 'Lj' = 'G', 'Bl' = 'H')) +
  scale_fill_manual(values = c('core' = 'white', 'edge' = 'black'), labels = c('edge' = "South", 'core' = "North")) +
  theme_bw(base_size = 15) +
...
emtrends(modAllSamplat, specs = ~treatment*core_edge, var = "latitude", type = 'response')
emmip(modAllSamplat, treatment | core_edge ~ latitude, cov.reduce = range, CIs = F)
...
emmip(logmodel2, treatment | core_edge ~ latitude, cov.reduce = range)
...
##Combining plots for publication
ggarrange(prop_coreedge_plot, prop_population_plot, 
          common.legend = T, 
          legend = 'right', 
          widths = c(1.3,2),
          labels = 'AUTO')
